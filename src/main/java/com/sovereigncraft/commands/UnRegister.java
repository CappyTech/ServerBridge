...
public class UnRegister implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            player.sendMessage("§cUsage: /chat unregister <password>");
            return true;
        }
        ...
        if (matrixUsername == null) {
            player.sendMessage("§cNo Matrix registration found for you. Try /chat register <password>.");
            return true;
        }
        ...
        new Thread(() -> {
            try {
                ...
                String json = "{" +
                        "\"auth\": {\"type\": \"m.login.password\", \"user\": \"" + effectiveUsername + "\", \"password\": \"" + password + "\"}," +
